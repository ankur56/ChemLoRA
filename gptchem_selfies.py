...
def pickle_to_data(filename):
    ...
    return selfies_list, b3lyp_at_list, g4mp2_at_list, en_diff_list


train_selfies_list, train_b3lyp_at_list, train_g4mp2_at_list, train_en_diff_list = pickle_to_data(
    "pickles/qm9_key_selfies_1_full_train_data.pickle")
holdout_selfies_list, holdout_b3lyp_at_list, holdout_g4mp2_at_list, holdout_en_diff_list = pickle_to_data(
    "pickles/qm9_key_selfies_1_holdout_data.pickle")
#sugar_selfies_list, sugar_b3lyp_at_list, sugar_g4mp2_at_list, sugar_en_diff_list = pickle_to_data("pickles/sugar_data_key_selfies_full.pickle")
...
regressor.fit(train_selfies_list, train_g4mp2_at_list)
#regressor.fit(train_selfies_list, train_en_diff_list)

# Make predictions using the holdout set
y_pred_holdout = regressor.predict(holdout_selfies_list)
y_pred_holdout_list = y_pred_holdout.tolist()
y_true_holdout = holdout_g4mp2_at_list
#y_true_holdout = holdout_en_diff_list

#y_pred_sugar = regressor.predict(sugar_selfies_list)
#y_pred_sugar_list = y_pred_sugar.tolist()
#y_true_sugar = sugar_g4mp2_at_list
#y_true_sugar = sugar_en_diff_list

results = {'y_pred_holdout': y_pred_holdout_list, 'y_true_ho': y_true_holdout}
...
with open('gptchem_results_selfies_g4mp2_y_all.json', 'w') as f:
    #with open('gptchem_results_selfies_en_diff_y_all.json', 'w') as f:
    json.dump(results, f)
